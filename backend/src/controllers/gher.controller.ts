...
const setupSchema = z.object({ count: z.number().int().positive().max(60) });
...
export async function updateStatus(req: AuthedRequest, res: Response) {
  ...
  const gher = await updateGherStatus(req.kiln!.id, req.params.id, input.status);
  res.json(gher);
}

export async function roundSpeed(req: AuthedRequest, res: Response) {
  const days = req.query.days ? Number(req.query.days) : 14;
  const result = await fireRoundSpeed(req.kiln!.id, days);
  res.json(result);
}
