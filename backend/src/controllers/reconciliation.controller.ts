...
export async function get(req: AuthedRequest, res: Response) {
  ...
  const result = await reconcileSoilToKiln(req.kiln!.id, days);
  res.json(result);
}

export async function finishedGoods(req: AuthedRequest, res: Response) {
  const days = req.query.days ? Number(req.query.days) : 30;
  const result = await reconcileFinishedGoods(req.kiln!.id, days);
  res.json(result);
}

export async function dashboardStock(req: AuthedRequest, res: Response) {
  const result = await dashboardStockSummary(req.kiln!.id);
  res.json(result);
}
