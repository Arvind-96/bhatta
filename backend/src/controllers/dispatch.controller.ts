...
export async function soldByCategory(req: AuthedRequest, res: Response) {
  const seasonIds = await seasonIdsThrough(req.kiln!.id, req.season!.id);
  const result = await bricksSoldByCategory(req.kiln!.id, seasonIds);
  res.json(result);
}
