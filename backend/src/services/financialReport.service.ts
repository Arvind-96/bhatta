...
import { dispatches, expenses, ledgerEntries, people, ghers, fuelLogs, stackingEntries, fuelPurchases } from "../db/schema";
...
export async function seasonFinancialSummary(kilnId: string, days = 30) {
  ...
  const revenue = dispatchRows.reduce((sum, d) => sum + d.amount, 0);
