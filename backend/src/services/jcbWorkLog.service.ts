...
export async function createJcbWorkLog(input: CreateJcbWorkLogInput) {
  ...
  if (input.machineId) {
    const machine = db.select().from(machines).where(and(eq(machines._id, input.machineId), eq(machines.kilnId, input.kilnId))).get();
  ...
  if (input.contractId) {
    const contract = db.select().from(soilContracts).where(and(eq(soilContracts._id, input.contractId), eq(soilContracts.kilnId, input.kilnId))).get();
  ...
  db.insert(jcbWorkLogs).values({ ...input, _id }).run();
  const log = db.select().from(jcbWorkLogs).where(eq(jcbWorkLogs._id, _id)).get()!;
...
export async function listJcbWorkLogs(kilnId: string, filter: ListJcbWorkLogsFilter = {}) {
  ...
  const rows = await db.select().from(jcbWorkLogs).where(and(...conditions)).orderBy(desc(jcbWorkLogs.date)).all();
  ...
  const [landRows, personRows, machineRows] = await Promise.all([
    landIds.length ? db.select({ _id: lands._id, name: lands.name, village: lands.village }).from(lands).where(inArray(lands._id, landIds)).all() : [],
    personIds.length ? db.select({ _id: people._id, name: people.name }).from(people).where(inArray(people._id, personIds)).all() : [],
    machineIds.length ? db.select({ _id: machines._id, name: machines.name, identifier: machines.identifier }).from(machines).where(inArray(machines._id, machineIds)).all() : [],
  ]);
...
export async function totalJcbHoursForLand(kilnId: string, landId: string) {
  const logs = await db.select().from(jcbWorkLogs).where(and(eq(jcbWorkLogs.kilnId, kilnId), eq(jcbWorkLogs.landId, landId))).all();
