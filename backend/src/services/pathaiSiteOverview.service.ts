...
export async function pathaiSiteOverview(kilnId: string, seasonId: string) {
  ...
  // Only PHAD_TO_STOCK actually removes bricks from a site's own raw stock
  // — it covers bricks going from the Phad (molding ground, at the site)
  // to either the kiln's stock pile or straight into the chamber, either
  // way leaving the site. STOCK_TO_CHAMBER is a purely kiln-side move
  // (already-staged stock -> chamber) that never touches a Pathai site's
  // own siteId, so it's excluded here — counting it too would subtract
  // the same bricks from the site's stock twice.
    ...
    db
      ...
      .where(
        and(
          ...
          eq(stackingEntries.stage, "PHAD_TO_STOCK")
        )
      ),
