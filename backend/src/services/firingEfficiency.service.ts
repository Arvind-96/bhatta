...
export async function fuelEfficiency(kilnId: string, seasonId: string, days = 7, baselineDays = 30) {
  ...
  const [recentFuel, recentA1, baselineFuel, baselineA1] = await Promise.all([
    totalFuelConsumed(kilnId, [seasonId], recentSince),
    totalA1Output(kilnId, [seasonId], recentSince),
    totalFuelConsumed(kilnId, [seasonId], baselineSince),
    totalA1Output(kilnId, [seasonId], baselineSince),
  ]);
